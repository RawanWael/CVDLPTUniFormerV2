        ...
        with g_pathmgr.open(path_to_checkpoint, "rb") as f:
            ...
            while n_try < 10:
                try:
                    ...
                    n_try += 1
                    time.sleep(10)
                    logger.info(f'{n_try}-th try failed, wait 10 s......')
        model_state_dict_3d = (
...
def load_test_checkpoint(cfg, model, backup=False):
    """
    ...
    if cfg.TEST.CHECKPOINT_FILE_PATH != "":
        ...
        backup_checkpoint = os.path.join(cfg.OUTPUT_DIR, "checkpoints", "checkpoint_backup.pyth")
        load_checkpoint(backup_checkpoint, model, None, cfg.NUM_GPUS > 1)
